...
export async function generateCommand(description: string, options: { output?: string; url?: string; model?: string }): Promise<void> {
  ...
  if (options.url) console.log(` Base URL: ${options.url}`);
  ...
  try {
    ...
    const test: GeneratedTest = JSON.parse(content);
...
function generatePlaywrightCode(test: GeneratedTest, baseUrl?: string): string {
